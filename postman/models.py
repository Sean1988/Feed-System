...
from postman.utils import email_visitor, email_corporate, notify_user
...
class Message(models.Model):
    ...
    def notify_users(self, initial_status, is_auto_moderated=True,msgType=None):
        ...
        if initial_status == STATUS_PENDING:
            if self.is_rejected():
                ...
                if self.recipient.is_company == True:

                    try:
                        email_corporate(self, 'acceptance',msgType)
                    except:
                        pass
                else:
                    try:
                        email_visitor(self, 'acceptance')
                    except:
                        print "can not email"
