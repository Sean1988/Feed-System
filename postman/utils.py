...
def email_visitor(object, action):
    ...
    msg = EmailMultiAlternatives(subject, text_content,settings.DEFAULT_FROM_EMAIL, [object.recipient.email])
    msg.attach_alternative(html_content, "text/html")
    msg.send()
    #email('postman/email_visitor_subject.txt', 'postman/email_visitor.txt', [object.email], object, action)

from django.core.mail import EmailMultiAlternatives
def email_corporate(object, action,msgType):
    subject = "%s inquiry from %s" %(msgType, object.sender.first_name)
    signer = Signer(salt='blastoffCorp')
    token = signer.sign(object.recipient.email)
    url = "http://www.blastoff.co/accounts/register/?token="+ token +"&next=/message/inbox/"
    ctx_dict = { 'msgType': msgType, 'object': object, 'action': action,'url':url}
    text_content = ""
    html_content = render_to_string('postman/email_corp.html', ctx_dict)
    print "send email"
    msg = EmailMultiAlternatives(subject, text_content,settings.DEFAULT_FROM_EMAIL, [object.recipient.email])
