...
def mod1(message):
    print message.recipient.email
    print message.recipient.email.split('@')[0]
    if message.recipient.email.split('@')[0] == "unknow":
        return None
    return True
...
urlpatterns = patterns('postman.views',
    ...
    url(r'^reply/(?P<message_id>[\d]+)/$', 'reply',{'auto_moderators': mod1}, name='postman_reply'),
