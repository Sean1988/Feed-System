...
class BasicCommaSeparatedUserField(CharField):
    ...
    def clean(self, value):
        ...
        #print user_model.USERNAME_FIELD
        #print "BBBBB"
        USERNAME_FIELD = 'slug'
        users = list(user_model.objects.filter(is_active=True, **{'{0}__in'.format(USERNAME_FIELD): names}))
        unknown_names = set(names) ^ set([u.slug for u in users])
        errors = []
        if unknown_names:
